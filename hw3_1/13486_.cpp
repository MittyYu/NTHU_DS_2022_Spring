...
template < class T >
Manager<T>::Manager():endTime(-1){};

template < class T >
int Manager<T>::finish_in_time(u32 costTime,u32 deadLine)
{
    u32 tmp = costTime + taQue.get_min().get_time();
    if(end_flag != 0)
        return (tmp < deadLine) && (tmp <= endTime);
    else
        return tmp < deadLine;
};
...
void Manager<T>::cmd_add_task(string taskName, u32 costTime){
    if(taQue.size() < 1){
        ...
        curTime = std::max(curTime, tmp_time);
        
        Node newNode = Node(tmp_id, tmp_time);
        taQue.del_min();
